...
- **Sugestão de descrição** (só despesas): agrupa por `lower(descricao)`, ordena por número de usos e desempata pela mais recente, e traz — **tudo da mesma linha mais recente daquela descrição, numa única consulta** — `subcategoria_id`, `conta_id`, `pessoa_id`, o último valor, a `prioridade` e a **sobrescrita de essencialidade**. A sobrescrita não existe como coluna: `vw_despesas` expõe só a essencialidade efetiva, então ela é derivada com `NULLIF(v.essencialidade, s.essencialidade)` sobre um `JOIN tb_subcategorias s ON s.id = v.subcategoria_id` — dá `NULL` tanto para quem herdou da subcategoria quanto para quem sobrescreveu com o mesmo valor, e sobrescrita inócua não precisa viajar até a tela. O `JOIN` é 1:1 (FK para a PK) e por isso não altera as janelas de contagem e de recência. Receitas não têm autocomplete (decisão da rodada 7: poucas fontes, o select resolve).
...
| `integra_ipca` | `BOOLEAN NOT NULL DEFAULT TRUE` | Se entra no agregado de despesas deflacionado. `FALSE` para gastos pontuais que distorceriam a série. **Um único lugar do sistema lê esta coluna** (rodada 26): o recorte da **Análise por prioridade** (`/analise/prioridade`), que soma só as marcadas — inclusive com a correção pelo IPCA desligada — e diz na tela quanto ficou de fora. A **Análise por subcategoria** continua **não** a consultando, por decisão do dono: a série de uma subcategoria é a subcategoria inteira. Até a rodada 25 nada a lia. |
...
| Despesa deflacionada | `valor × indice_base / indice_do_mes`, por lançamento, somada depois; `indice_base` é o do último mês carregado em `tb_ipca`. A mesma expressão serve às duas análises: a por subcategoria (rodada 24), que **não filtra por `integra_ipca`**, e a por prioridade (rodada 26), que filtra. Nesta, o filtro é `FILTER (WHERE v.integra_ipca)` nas somas — e não no `WHERE` —, para o total do que ficou de fora sair da mesma varredura por `GROUPING SETS ((mes), ())` |
